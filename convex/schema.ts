...
// Define the nested schemas for the single trip document
const placeSchema = v.object({
    ...
    googleMapsUrl: v.string(), // REQUIRED - Google Maps URL is mandatory
...
const transportationSchema = v.object({
    type: v.union(
        v.literal("walk"),
        v.literal("taxi"),
        v.literal("uber"),
...
const daySchema = v.object({
    ...
    transportation: v.array(transportationSchema),
});

export default defineSchema({
    // Single document containing entire trip
    trips: defineTable({
        name: v.string(),
        description: v.optional(v.string()),
